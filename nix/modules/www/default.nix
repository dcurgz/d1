...
{
  flake.modules.nixos.personal-websites = flake.lib.nixos.mkAspect (with flake.tags; [ nixos-base ])
    ({
      lib,
      config,
      pkgs,
    ...
    let
      secrets = config.by.git-secrets.weirdfish-acme;
        ...
        sites = lib.mkOption {
          type = lib.types.attrsOf (lib.types.submodule ({ config, name, ... }: {
            options = {
              ...
              acme.enable = lib.mkEnableOption "Enable ACME automatic SSL certificate generation.";
              cloudflare.enable = lib.mkEnableOption "Enable Cloudflare X-Real-IP configuration.";
              anubis = {
                enable = lib.mkEnableOption "Enable Anubis, a filter-proxy, for this website.";
                ports.bind = lib.mkOption {
                  type = lib.types.int;
                  description = "The tcp port for Anubis to bind on.";
                };
                ports.target = lib.mkOption {
                  type = lib.types.int;
                  description = "The tcp port for Anubis to target.";
                };
                instanceOptions = lib.mkOption {
              ...
              web-server = {
                enable = lib.mkEnableOption "Enable a static web-server (nginx) to serve content post-filtering.";
      ...
      config = lib.mkIf (cfg.enable) {
        ...
        services = {
          nginx = {
            ...
            package = lib.mkIf (cfg.debug) (with pkgs; nginx.override (_prev: { withDebug = true; }));
            logError = lib.mkIf (cfg.debug) "/var/log/nginx/error.log debug";

            virtualHosts = lib.mkMerge (lib.mapAttrsToList (_sitename: site: {
              "${site.domain}" =
                if (site.anubis.enable) then
                {
                  ...
                  locations."/" = {
                    ...
                    extraConfig = lib.mkIf (site.cloudflare.enable) ''
              ...
              "${site.domain}-post-filter" = lib.mkIf (site.anubis.enable && site.web-server.enable) {
                listen = [
                  {
                    addr = "[::1]";
          ...
          anubis.instances = lib.mkMerge (lib.mapAttrsToList (_sitename: site: {
            "${site.domain}" = lib.mkIf (site.anubis.enable) {
              enable = true;
              settings = {
